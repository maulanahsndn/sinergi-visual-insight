  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden cinematic-hero">
      ...
      <div 
        ...
        style={{ backgroundImage: `url(/lovable-uploads/bc55cc81-4345-4ab7-8c67-840d506ae6d8.png)` }}
      />
      {/* Background Overlay */}
      ...
      >
        ...
        >
          <Rocket className="w-8 h-8 text-primary animate-glow-pulse" />
        ...
        <motion.h1 
          ...
          className="text-5xl md:text-7xl lg:text-8xl font-poppins font-bold mb-6 leading-tight"
        >
          <span className="bg-gradient-to-r from-foreground via-primary-glow to-accent bg-clip-text text-transparent">
            Laporan Interview
          </span>
          <br />
          <span className="text-foreground/80">Masa Depan</span>
        </motion.h1>
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="glass-card px-8 py-4 text-lg font-semibold text-primary hover:neon-glow transition-all duration-300 hover-lift"
          ...
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="glass-button px-8 py-4 text-lg font-medium hover-lift"
        ...
        >
          {[
            { number: "30+", label: "Laporan Interview", icon: Users },
            { number: "100%", label: "Responsif", icon: Sparkles },
            { number: "AI", label: "Powered Assistant", icon: Rocket }
          ].map((stat, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, y: 20 }}
